...
- **`pringle/cell_widget.py`**: `CellTextEdit` gains `indent_at`, `outdent_at`, `move_up_at`, `move_down_at` signals; `keyPressEvent` handles `ControlModifier+Key_BracketRight/Left` (indent/outdent) and `AltModifier & Key_Up/Down` (Option+Arrow to reorder). Uses bitwise `mod & alt` rather than equality to tolerate platform-extra modifier bits on macOS. `CellWidget` gains the matching four `*_requested` signals wired from `_text_edit`.
- **`pringle/comment_cell_widget.py`**: `_CommentEdit` gains all four signals and the same key handling. `CommentCellWidget` gains the four `*_requested` signals wired from `_edit`.
- **`pringle/slider_widget.py`**: `_SpinBox`, `_ExprBox`, and `_NameLineEdit` (the transient rename field) all gain the four signals and the same key handling. `SliderWidget` gains the four `*_requested` signals; `_build_ui` connects all four numeric fields, and `_on_name_clicked` connects the transient `_name_edit` each time it is created.
