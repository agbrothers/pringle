...
When all tests pass, move items out of the backlogs and into the closed docs (`16-closed-bugs.md`, `17-closed-features.md`, `19-closed-performance.md`). Then proceed to update any other relevant design-docs and commit the changes via git. Recording material changes to the architecture or any other assumptions is of paramount importance. The docs serve as the project memory, and a disconnect between the memory and the code can completely derail development. 
