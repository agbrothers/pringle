...
**`CellTextEdit` auto-height.** `_on_document_size_changed` sums `blockBoundingRect(block).height()` over every block — exact pixel heights from the layout engine, not a font-metrics estimate — and calls `setFixedHeight`. It is connected to `documentSizeChanged` (fires on content changes) **and** called explicitly from `resizeEvent` after `super()`. The explicit call handles the case where `documentSizeChanged` is deferred (Qt build/platform dependent): `super()` always runs a synchronous document relayout so `blockBoundingRect` is accurate immediately after it returns, regardless of whether the signal has fired yet. If the signal already ran synchronously inside `super()`, the explicit call is a no-op (identical computation). `_adjust_height()` (font-metrics estimate) is only called from `__init__` as a bootstrap before the first layout pass.
