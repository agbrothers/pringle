...
| Data-mode cell skip gate in `_rebuild_namespace` | Each rebuild pass tracks `changed_ids` (cells whose output changed). Sliders: value-compared against `_shared_ns`. Equation cells: hashed as `hash((source, constraint_exprs, condition_exprs, recurrence_expr, initial_condition_exprs, _rng_seed))`; self_changed = stored hash ≠ current hash. Data-mode cells are skipped if neither self_changed nor any DAG ancestor is in `changed_ids`; their cached `_last_result.exports` are injected into `shared` as-is. If the grid config (n, bounds) changed since the last rebuild, `prev_hashes` is cleared so all cells appear new and the skip gate does not fire. `→` button increments `_rng_seed`, which changes the hash, guaranteeing re-evaluation. | PERF-016: reduces add/remove/drag rebuild time in `memory-clustering.yml` from ~8,737 ms to ~415 ms by skipping expensive recurrence cells when their inputs are unchanged. Grid-config invalidation prevents stale exports from cells that use `x`/`y` magic variables (e.g. `grid = array([x,y]).reshape(2,-1)`). |
