...
| Expression security | Session trust model: file-loaded sessions start unexecuted; play button overlay requires explicit user approval before `_rebuild_namespace()` runs. New sessions are trusted by default. No sandbox or AST check — full Python available after trust. | Mirrors Jupyter/VS Code model; enables arbitrary imports and porting of existing scientific code; see `03-expression-evaluation.md` |
...
| Python script export | **Export** button (Ctrl+Shift+E) writes a standalone `.py` via `pringle/export.py`. Cells emitted in DAG topological order; sliders become plain assignments; recurrence cells expand to `for n in range(1, len(arr)):` loops; imports are minimal (only names actually referenced, detected via `get_free_names`); spatial runtime variables (`x`, `y`, `cfg`, `camera`) get a setup block when referenced. Folder/comment placement uses Option B: folders anchor to their first topo-ordered member; comments anchor to the next evaluable cell in visual order. |
...
| Session boot sequence | (1) load YAML, (2) restore cell list (`_session_trusted=False`), (3) show play button overlay — no evaluation yet; (4) user clicks play → `_session_trusted=True` → `_rebuild_namespace()` → first render | File-loaded sessions are never auto-executed; user must approve before any cell runs. New sessions skip the play button (trusted by default). |
...
| Camera fit on add | `PringleViewport` calls `fit_camera()` only when the scene transitions from **empty to non-empty** (`_seen_cell_ids` was empty before the call). Subsequent new cells (duplicates, session evaluation, user additions) never re-fit. `_on_open` pre-populates `_seen_cell_ids` from all loaded cell IDs before play-button evaluation fires, preventing the restored camera position from being overwritten. | BUG-180: prevents session load and cell duplicate from resetting the camera |
